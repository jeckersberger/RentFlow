...
func NewPolicyPostgres(db *sql.DB) *PolicyPostgres {
...
func (r *PolicyPostgres) ListActive(ctx context.Context, tenantID string) ([]*domain.Policy, error) {
	query := `
		...
		ORDER BY created_at DESC
	`
	rows, err := r.db.QueryContext(ctx, query, tenantID, domain.PolicyStatusActive)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *PolicyPostgres) ListExpiring(ctx context.Context, tenantID string, days int) ([]*domain.Policy, error) {
	...
	`
	rows, err := r.db.QueryContext(ctx, query, tenantID, domain.PolicyStatusActive, days)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func NewClaimPostgres(db *sql.DB) *ClaimPostgres {
...
func (r *ClaimPostgres) ListByEquipment(ctx context.Context, tenantID, equipmentID string) ([]*domain.Claim, error) {
	query := `
		...
		ORDER BY created_at DESC
	`
	rows, err := r.db.QueryContext(ctx, query, tenantID, equipmentID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *ClaimPostgres) ListByPolicy(ctx context.Context, tenantID, policyID string) ([]*domain.Claim, error) {
	query := `
		...
		ORDER BY created_at DESC
	`
	rows, err := r.db.QueryContext(ctx, query, tenantID, policyID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func NewRiskAssessmentPostgres(db *sql.DB) *RiskAssessmentPostgres {
...
func (r *RiskAssessmentPostgres) GetByEquipment(ctx context.Context, tenantID, equipmentID string) (*domain.RiskAssessment, error) {
	...
	err := r.db.QueryRowContext(ctx, query, tenantID, equipmentID).Scan(
...
func (r *RiskAssessmentPostgres) Delete(ctx context.Context, tenantID, equipmentID string) error {
	...
	_, err := r.db.ExecContext(ctx, query, tenantID, equipmentID)
	return err
}
