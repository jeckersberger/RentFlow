...
import (
	...
	"github.com/jeckersberger/EquipFlow/services/inventory/internal/adapters/httphandler"
	"github.com/jeckersberger/EquipFlow/services/inventory/internal/application"
	"github.com/jeckersberger/EquipFlow/services/inventory/internal/infrastructure/postgres"
)

func main() {
	// 1. Load configuration with inventory-service defaults.
	setDefaults()
	cfg := config.Load()

	// 2. Create logger.
	log := logger.New(cfg.LogLevel)
	log = logger.WithService(log, "inventory")
	...
	addr := ":" + cfg.Port
	log.Info().Str("addr", addr).Msg("inventory-service starting")
	...
	log.Info().Msg("inventory-service stopped")
}

// setDefaults overrides environment variable defaults for the inventory-service.
func setDefaults() {
	if os.Getenv("DB_NAME") == "" {
		os.Setenv("DB_NAME", "inventory_service")
	}
	if os.Getenv("PORT") == "" {
		os.Setenv("PORT", "8002")
	}
...
func generateDevPublicKey() (string, error) {
	...
	pubPath := "/tmp/cratedesk_inventory_public.pem"
