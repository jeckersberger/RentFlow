...
import (
	...
	"github.com/jeckersberger/EquipFlow/services/reporting/internal/adapters/httphandler"
	"github.com/jeckersberger/EquipFlow/services/reporting/internal/application"
	"github.com/jeckersberger/EquipFlow/services/reporting/internal/infrastructure/postgres"
)

func main() {
	// 1. Load configuration with reporting-service defaults.
	setDefaults()
	cfg := config.Load()

	// 2. Create logger.
	log := logger.New(cfg.LogLevel)
	log = logger.WithService(log, "reporting")
	...
	addr := ":" + cfg.Port
	log.Info().Str("addr", addr).Msg("reporting-service starting")
	...
	log.Info().Msg("shutting down reporting-service...")
	...
	log.Info().Msg("reporting-service stopped")
}

// setDefaults overrides environment variable defaults for the reporting-service.
func setDefaults() {
	if os.Getenv("DB_NAME") == "" {
		os.Setenv("DB_NAME", "reporting_service")
	}
	if os.Getenv("PORT") == "" {
		os.Setenv("PORT", "8016")
	}
...
func generateDevPublicKey() (string, error) {
	...
	pubPath := "/tmp/cratedesk_reporting_public.pem"
