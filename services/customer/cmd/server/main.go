...
import (
	...
	"github.com/jeckersberger/EquipFlow/services/customer/internal/adapters/httphandler"
	"github.com/jeckersberger/EquipFlow/services/customer/internal/application"
	"github.com/jeckersberger/EquipFlow/services/customer/internal/infrastructure/postgres"
)
...
func main() {
	...
	log = logger.WithService(log, "customer")
	...
	contactSvc := application.NewContactService(contactRepo, noteRepo, log)

	customerH := httphandler.NewCustomerHandler(customerSvc, log)
	contactH := httphandler.NewContactHandler(contactSvc, log)
	...
	addr := ":" + cfg.Port
	log.Info().Str("addr", addr).Msg("customer-service starting")
	...
	log.Info().Msg("shutting down customer-service...")
	...
	log.Info().Msg("customer-service stopped")
}

func setDefaults() {
	if os.Getenv("DB_NAME") == "" {
		os.Setenv("DB_NAME", "customer_service")
	}
	if os.Getenv("PORT") == "" {
		os.Setenv("PORT", "8019")
	}
...
func generateDevPublicKey() (string, error) {
	...
	pubPath := "/tmp/equipflow_customer_public.pem"
