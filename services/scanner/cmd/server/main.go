...
import (
	...
	"github.com/jeckersberger/EquipFlow/services/scanner/internal/adapters/httphandler"
	"github.com/jeckersberger/EquipFlow/services/scanner/internal/application"
	"github.com/jeckersberger/EquipFlow/services/scanner/internal/infrastructure/postgres"
)

func main() {
	// 1. Load configuration with scanner-service defaults.
	setDefaults()
	cfg := config.Load()

	// 2. Create logger.
	log := logger.New(cfg.LogLevel)
	log = logger.WithService(log, "scanner")
	...
	addr := ":" + cfg.Port
	log.Info().Str("addr", addr).Msg("scanner-service starting")
...
// setDefaults overrides environment variable defaults for the scanner-service.
func setDefaults() {
	if os.Getenv("DB_NAME") == "" {
		os.Setenv("DB_NAME", "scanner_service")
	}
	if os.Getenv("PORT") == "" {
		os.Setenv("PORT", "8004")
	}
...
func generateDevPublicKey() (string, error) {
	...
	pubPath := "/tmp/equipflow_scanner_public.pem"
