...
import (
	...
	"github.com/jeckersberger/EquipFlow/services/project/internal/adapters/httphandler"
	"github.com/jeckersberger/EquipFlow/services/project/internal/application"
	"github.com/jeckersberger/EquipFlow/services/project/internal/infrastructure/postgres"
)
...
func main() {
	...
	log = logger.WithService(log, "project")
	...
	peRepo := postgres.NewProjectEquipmentRepo(pool)
	packlistRepo := postgres.NewPacklistRepo(pool)
	...
	// HTTP handlers
	projectH := httphandler.NewProjectHandler(projectSvc, log)
	packlistH := httphandler.NewPacklistHandler(packlistSvc, log)
	reservationH := httphandler.NewReservationHandler(reservationSvc, log)
	...
	router := httphandler.NewRouter(
		projectH, packlistH, reservationH,
		healthH, livenessH,
		jwtMW, corsMW, recoveryMW, requestIDMW,
	)

	addr := ":" + cfg.Port
	log.Info().Str("addr", addr).Msg("project-service starting")
	...
	log.Info().Msg("project-service stopped")
}

func setDefaults() {
	if os.Getenv("DB_NAME") == "" {
		os.Setenv("DB_NAME", "project_service")
	}
	if os.Getenv("PORT") == "" {
		os.Setenv("PORT", "8003")
	}
...
func generateDevPublicKey() (string, error) {
	...
	pubPath := "/tmp/cratedesk_project_public.pem"
