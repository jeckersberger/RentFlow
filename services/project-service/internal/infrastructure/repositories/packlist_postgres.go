...
func (r *PacklistPostgres) Update(ctx context.Context, p *domain.Packlist) error {
	...
	_, err = tx.Exec(ctx, `DELETE FROM projects.packlist_items WHERE packlist_id = $1`, p.ID)
	if err != nil {
...
func (r *PacklistPostgres) Delete(ctx context.Context, tenantID, packlistID string) error {
	...
	// Delete items first
	_, err = tx.Exec(ctx, `DELETE FROM projects.packlist_items WHERE packlist_id = $1`, packlistID)
	if err != nil {
		return fmt.Errorf("failed to delete items: %w", err)
	}

	// Delete packlist
	result, err := tx.Exec(ctx, `DELETE FROM projects.packlists WHERE id = $1 AND tenant_id = $2`, packlistID, tenantID)
	if err != nil {
