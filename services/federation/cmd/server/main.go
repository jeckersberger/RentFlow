...
import (
	...
	"github.com/jeckersberger/EquipFlow/services/federation/internal/adapters/httphandler"
	"github.com/jeckersberger/EquipFlow/services/federation/internal/application"
	"github.com/jeckersberger/EquipFlow/services/federation/internal/infrastructure/postgres"
)

func main() {
	// 1. Load configuration with federation-service defaults.
	setDefaults()
	cfg := config.Load()

	// 2. Create logger.
	log := logger.New(cfg.LogLevel)
	log = logger.WithService(log, "federation")
	...
	partnerRepo := postgres.NewPartnerRepo(pool)
	listingRepo := postgres.NewListingRepo(pool)
	requestRepo := postgres.NewRequestRepo(pool)

	// 7. Create application services.
	partnerSvc := application.NewPartnerService(partnerRepo, log)
	listingSvc := application.NewListingService(listingRepo, log)
	requestSvc := application.NewRequestService(requestRepo, log)

	// 8. Create HTTP handlers.
	partnerH := httphandler.NewPartnerHandler(partnerSvc, log)
	listingH := httphandler.NewListingHandler(listingSvc, log)
	requestH := httphandler.NewRequestHandler(requestSvc, log)
	...
	addr := ":" + cfg.Port
	log.Info().Str("addr", addr).Msg("federation-service starting")
	...
	log.Info().Msg("shutting down federation-service...")
...
// setDefaults overrides environment variable defaults for the federation-service.
func setDefaults() {
	if os.Getenv("DB_NAME") == "" {
		os.Setenv("DB_NAME", "federation_service")
	}
	if os.Getenv("PORT") == "" {
		os.Setenv("PORT", "8009")
	}
...
func generateDevPublicKey() (string, error) {
	...
	pubPath := "/tmp/cratedesk_federation_public.pem"
