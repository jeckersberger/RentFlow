...
func NewScheduleHandler(service *application.ScheduleService, logger zerolog.Logger) *ScheduleHandler {
	return &ScheduleHandler{
		service: service,
		logger:  logger.With().Str("handler", "schedule").Logger(),
	}
}
...
func (h *ScheduleHandler) Get(w http.ResponseWriter, r *http.Request) {
	...
	schedule, err := h.service.GetByID(r.Context(), id, claims.TenantID)
	if err != nil {
		errors.HandleError(w, err)
		return
	}
