...
import (
	...
	"github.com/jeckersberger/EquipFlow/services/maintenance/internal/adapters/httphandler"
	"github.com/jeckersberger/EquipFlow/services/maintenance/internal/application"
	"github.com/jeckersberger/EquipFlow/services/maintenance/internal/infrastructure/postgres"
)

func main() {
	// 1. Load configuration with maintenance-service defaults.
	setDefaults()
	cfg := config.Load()

	// 2. Create logger.
	log := logger.New(cfg.LogLevel)
	log = logger.WithService(log, "maintenance")
	...
	addr := ":" + cfg.Port
	log.Info().Str("addr", addr).Msg("maintenance-service starting")
	...
	log.Info().Msg("shutting down maintenance-service...")
	...
	log.Info().Msg("maintenance-service stopped")
}

// setDefaults overrides environment variable defaults for the maintenance-service.
func setDefaults() {
	if os.Getenv("DB_NAME") == "" {
		os.Setenv("DB_NAME", "maintenance_service")
	}
	if os.Getenv("PORT") == "" {
		os.Setenv("PORT", "8010")
	}
...
func generateDevPublicKey() (string, error) {
	...
	pubPath := "/tmp/cratedesk_maintenance_public.pem"
