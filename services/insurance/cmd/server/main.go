...
import (
	...
	"github.com/jeckersberger/EquipFlow/services/insurance/internal/adapters/httphandler"
	"github.com/jeckersberger/EquipFlow/services/insurance/internal/application"
	"github.com/jeckersberger/EquipFlow/services/insurance/internal/infrastructure/postgres"
)

func main() {
	// 1. Load configuration with insurance-service defaults.
	setDefaults()
	cfg := config.Load()

	// 2. Create logger.
	log := logger.New(cfg.LogLevel)
	log = logger.WithService(log, "insurance")
	...
	equipmentSvc := application.NewEquipmentService(equipmentRepo, policyRepo, log)
	claimSvc := application.NewClaimService(claimRepo, log)

	// 8. Create HTTP handlers.
	policyH := httphandler.NewPolicyHandler(policySvc, log)
	equipmentH := httphandler.NewEquipmentHandler(equipmentSvc, log)
	claimH := httphandler.NewClaimHandler(claimSvc, log)
	...
	addr := ":" + cfg.Port
	log.Info().Str("addr", addr).Msg("insurance-service starting")
	...
	log.Info().Msg("shutting down insurance-service...")
	...
	log.Info().Msg("insurance-service stopped")
}

// setDefaults overrides environment variable defaults for the insurance-service.
func setDefaults() {
	if os.Getenv("DB_NAME") == "" {
		os.Setenv("DB_NAME", "insurance_service")
	}
	if os.Getenv("PORT") == "" {
		os.Setenv("PORT", "8012")
	}
...
func generateDevPublicKey() (string, error) {
	...
	pubPath := "/tmp/cratedesk_insurance_public.pem"
