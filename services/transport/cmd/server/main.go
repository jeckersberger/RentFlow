...
import (
	...
	"github.com/jeckersberger/EquipFlow/services/transport/internal/adapters/httphandler"
	"github.com/jeckersberger/EquipFlow/services/transport/internal/application"
	"github.com/jeckersberger/EquipFlow/services/transport/internal/infrastructure/postgres"
)

func main() {
	// 1. Load configuration with transport-service defaults.
	setDefaults()
	cfg := config.Load()

	// 2. Create logger.
	log := logger.New(cfg.LogLevel)
	log = logger.WithService(log, "transport")
	...
	orderH := httphandler.NewOrderHandler(orderSvc, log)
	...
	addr := ":" + cfg.Port
	log.Info().Str("addr", addr).Msg("transport-service starting")
	...
	log.Info().Msg("shutting down transport-service...")
	...
	log.Info().Msg("transport-service stopped")
}

// setDefaults overrides environment variable defaults for the transport-service.
func setDefaults() {
	if os.Getenv("DB_NAME") == "" {
		os.Setenv("DB_NAME", "transport_service")
	}
	if os.Getenv("PORT") == "" {
		os.Setenv("PORT", "8011")
	}
...
func generateDevPublicKey() (string, error) {
	...
	pubPath := "/tmp/cratedesk_transport_public.pem"
