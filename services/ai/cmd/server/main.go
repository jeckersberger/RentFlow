...
import (
	...
	"github.com/jeckersberger/EquipFlow/services/ai/internal/adapters/httphandler"
	"github.com/jeckersberger/EquipFlow/services/ai/internal/application"
	"github.com/jeckersberger/EquipFlow/services/ai/internal/infrastructure/postgres"
)

func main() {
	// 1. Load configuration with ai-service defaults.
	setDefaults()
	cfg := config.Load()

	// 2. Create logger.
	log := logger.New(cfg.LogLevel)
	log = logger.WithService(log, "ai")
	...
	predRepo := postgres.NewPredictionRepo(pool)
	sugRepo := postgres.NewSuggestionRepo(pool)
	tdRepo := postgres.NewTrainingDataRepo(pool)

	// 7. Create application services.
	predSvc := application.NewPredictionService(predRepo, log)
	...
	addr := ":" + cfg.Port
	log.Info().Str("addr", addr).Msg("ai-service starting")
...
// setDefaults overrides environment variable defaults for the ai-service.
func setDefaults() {
	if os.Getenv("DB_NAME") == "" {
		os.Setenv("DB_NAME", "ai_service")
	}
	if os.Getenv("PORT") == "" {
		os.Setenv("PORT", "8014")
	}
...
func generateDevPublicKey() (string, error) {
	...
	pubPath := "/tmp/cratedesk_ai_public.pem"
