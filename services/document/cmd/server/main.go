...
import (
	...
	"github.com/jeckersberger/EquipFlow/services/document/internal/adapters/httphandler"
	"github.com/jeckersberger/EquipFlow/services/document/internal/application"
	"github.com/jeckersberger/EquipFlow/services/document/internal/infrastructure/postgres"
)
...
func main() {
	...
	log = logger.WithService(log, "document")
	...
	addr := ":" + cfg.Port
	log.Info().Str("addr", addr).Msg("document-service starting")
	...
	log.Info().Msg("shutting down document-service...")
	...
	log.Info().Msg("document-service stopped")
}

func setDefaults() {
	if os.Getenv("DB_NAME") == "" {
		os.Setenv("DB_NAME", "document_service")
	}
	if os.Getenv("PORT") == "" {
		os.Setenv("PORT", "8007")
	}
...
func generateDevPublicKey() (string, error) {
	...
	pubPath := "/tmp/cratedesk_document_public.pem"
