...
func main() {
	...
	// API routes (v1)
	router.HandleFunc("GET /api/v1/equipment", listEquipmentHandler)
	router.HandleFunc("POST /api/v1/equipment", createEquipmentHandler)
	router.HandleFunc("GET /api/v1/equipment/{id}", getEquipmentHandler)
	router.HandleFunc("GET /api/v1/categories", listCategoriesHandler)
...
func listEquipmentHandler(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)
	w.Write([]byte(`{"data":[],"message":"not yet implemented","service":"inventory-service"}`))
}
