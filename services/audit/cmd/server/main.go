...
import (
	...
	"github.com/jeckersberger/EquipFlow/services/audit/internal/adapters/httphandler"
	"github.com/jeckersberger/EquipFlow/services/audit/internal/application"
	"github.com/jeckersberger/EquipFlow/services/audit/internal/infrastructure/postgres"
)

func main() {
	// 1. Load configuration with audit-service defaults.
	setDefaults()
	cfg := config.Load()

	// 2. Create logger.
	log := logger.New(cfg.LogLevel)
	log = logger.WithService(log, "audit")
	...
	logRepo := postgres.NewAuditLogRepo(pool)
	policyRepo := postgres.NewAuditPolicyRepo(pool)
	...
	addr := ":" + cfg.Port
	log.Info().Str("addr", addr).Msg("audit-service starting")
...
// setDefaults overrides environment variable defaults for the audit-service.
func setDefaults() {
	if os.Getenv("DB_NAME") == "" {
		os.Setenv("DB_NAME", "audit_service")
	}
	if os.Getenv("PORT") == "" {
		os.Setenv("PORT", "8017")
	}
...
func generateDevPublicKey() (string, error) {
	...
	pubPath := "/tmp/cratedesk_audit_public.pem"
